function EnviaPrimaaMotorPago() {
 var mov = document.forms[0].all.hidfolio.value; 
	if (confirm('Está Seguro de que desea enviar \n la Prima de la Opción \n al Motor de Pagos?')) {
		...
		sender.Open('GET', 'bop_srv_envmp_pr.asp?id=' + mov + '&pdat=' + task.getTime(), false);
		sender.Send();
...
function EnviaPrimaaPago(mov) {
	if (confirm('Está Seguro de que desea Procesar el Pago de la Prima \n Asociada a la Opción ' + mov + '?')) {
		var task = new Date()
		var sender = new ActiveXObject("Microsoft.XMLHTTP");
		sender.Open('GET', 'bop_srv_envpago_pr.asp?id=' + mov + '&pdat=' + task.getTime(), false);
		sender.Send();
		var answarr = sender.responseText.split("|");
		if (answarr[0] != '0') {
			alert(HtmlDecode(answarr[1]))		
