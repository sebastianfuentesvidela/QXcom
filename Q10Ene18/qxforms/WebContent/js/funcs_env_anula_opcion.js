function EnviaAnulacionOpcion() {
 var mov = document.forms[0].all.hidfolio.value; 
	if (confirm('Está Seguro de \n Anular Completamente esta Opcion? ')) {
		var task = new Date()
		var sender = new ActiveXObject("Microsoft.XMLHTTP");
		sender.Open('GET', 'bop_srv_anulacion_opcion.asp?id=' + mov + '&pdat=' + task.getTime(), false);
		sender.Send();
		...
		if (answarr[0] != '0') {
			...
			alert('No Fué Posible Anular la Opción:\n ' + HtmlDecode(answarr[1])) 
		}
